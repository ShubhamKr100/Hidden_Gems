...
function MyBookingsPage() {
  ...
  // State to track which booking's review form is open
  const [showReviewFormFor, setShowReviewFormFor] = useState(null);
  // State to track which bookings have been reviewed
  ...
  const handlePayment = async (bookingId) => {
    try {
      ...
      // User ko Stripe ke secure payment page par redirect kar do 🚀
  ...
  // Function called by ReviewForm on successful submission
  const handleReviewSubmitted = (bookingId) => {
            ...
            <div className="service-card booking-card" key={booking._id} style={{ marginBottom: '1.5rem', borderLeft: `5px solid ${booking.status === 'Pending' ? 'orange' : booking.status === 'Confirmed' ? 'green' : 'red'}` }}>
              ...
              {booking.status === 'Confirmed' && !booking.isPaid && (
                <button 
                  ...
                  style={{
                    backgroundColor: '#635BFF', // Stripe Purple Color
                    color: 'white',
                    padding: '10px 20px',
                    border: 'none',
                    borderRadius: '6px',
                ...
                >
                  💳 Pay ₹{booking.totalPrice} via Stripe
                </button>
              )}

              {/* Agar payment ho chuki hai toh Green Badge */}
              {booking.isPaid && (
                <span style={{ 
                  backgroundColor: '#d4edda', color: '#155724', padding: '6px 12px', 
                  borderRadius: '4px', fontWeight: 'bold', display: 'inline-block', marginTop: '10px' 
                }}>
                  Paid ✅ ({new Date(booking.paidAt).toLocaleDateString()})
                </span>
              )}
              {/* --- END: Stripe Payment Logic --- */}

              {/* --- Review Button and Form Logic --- */}
              {booking.status === 'Confirmed' && !reviewedBookings.has(booking._id) && (
                 <div>
                   {showReviewFormFor !== booking._id && (
                     <button
                       className="cta-button"
                       onClick={() => setShowReviewFormFor(booking._id)}
                       style={{ marginTop: '1rem', backgroundColor: '#ff9900' }}
                     >
                       Leave a Review
                     </button>
                   )}
